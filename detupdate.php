<?php 
 session_start(); //starts the session
    if($_SESSION['user']){ // checks if the user is logged in  
       ...
       header("location:admin.php"); // redirects if user is not logged in
   }
echo "
<html>
    <head>
        ...
        <form action='' method='POST'>
         DOCTOR NAME: <input type='text' name='doctor'  required='required' value=".$_GET['doctor']."> <br/>
           ...
           DEPARTMENT NAME: <input type='text' name='department'  required='required' value=".$_GET['department']."> <br/>
            ...
            password: <input type='text' name='password'  required='required' value=".$_GET['password']."> <br/>
...
$sql= "UPDATE `details` SET `doctor`='$doctor',`hospital`='$hospital',`department`='$department',`number`='$number',`email`='$email',`password`='$password'  WHERE `id`='$id'";
